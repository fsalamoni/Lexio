...
export default function NewDocument() {
  ...
  const handleProgress = useCallback((p: GenerationProgress) => {
    ...
    if (completed) {
      setPipelineAgents(prev =>
        prev.map(a =>
          a.status === 'active'
            ? { ...a, status: 'completed' as const, executionState: 'completed' as const, completedAt: now }
            : a,
        ),
      )
  ...
  const handleDetailContext = async () => {
    ...
    try {
      const result = await generateContextQuestions(selectedType, request, selectedAreas, userId || undefined)
      setContextDetail({
