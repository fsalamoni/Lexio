...
export type NotebookContentBackfillReport = {
  scanned: number
  migrated: number
  already_dedicated: number
  empty_legacy: number
...
const MIN_ARTIFACT_CONTENT_CHARS = 240
...
function fitArtifactsToFirestoreLimit(
  artifacts: StudioArtifact[],
  otherDataEstimateBytes: number,
): { artifacts: StudioArtifact[]; truncated: boolean } {
  const totalBytes = estimateJsonBytes(artifacts) + otherDataEstimateBytes
  if (totalBytes <= NOTEBOOK_MAX_DOC_BYTES) return { artifacts, truncated: false }
  ...
  if (estimateJsonBytes(retained) + otherDataEstimateBytes <= NOTEBOOK_MAX_DOC_BYTES) {
    console.warn(
      `[Lexio] Notebook artifacts trimmed to fit Firestore 1 MB limit ` +
      `(removed ${artifacts.length - retained.length} oldest artifacts; kept ${retained.length}/${artifacts.length} most recent artifacts).`,
    )
    return { artifacts: retained, truncated: true }
  }

  const budget = Math.max(NOTEBOOK_MAX_DOC_BYTES - otherDataEstimateBytes, 0)
  const totalContentChars = retained.reduce((sum, artifact) => sum + artifact.content.length, 0)
  if (totalContentChars === 0) return { artifacts: retained, truncated: true }

  const metaOverhead = estimateJsonBytes(retained) - Math.ceil(totalContentChars * 1.1)
  const availableForContent = Math.max(budget - metaOverhead, 0)
  const ratio = availableForContent / Math.ceil(totalContentChars * 1.1)

  const fittedArtifacts = retained.map(artifact => {
    if (artifact.content.length === 0 || ratio >= 1) return artifact
    const maxChars = Math.max(Math.floor(artifact.content.length * ratio), MIN_ARTIFACT_CONTENT_CHARS)
    if (maxChars >= artifact.content.length) return artifact
    return { ...artifact, content: artifact.content.slice(0, maxChars) }
  })

  console.warn(
    `[Lexio] Notebook artifact content trimmed to fit Firestore 1 MB limit ` +
    `(estimated ${(totalBytes / 1024).toFixed(0)} KiB -> budget ${(NOTEBOOK_MAX_DOC_BYTES / 1024).toFixed(0)} KiB).`,
  )

  return { artifacts: fittedArtifacts, truncated: true }
}

function hasNotebookContentPayload(data: Partial<Pick<ResearchNotebookData, 'messages' | 'artifacts'>>): boolean {
  return data.messages !== undefined || data.artifacts !== undefined
}

function hasNonEmptyLegacyContent(notebook: ResearchNotebookData): boolean {
  return (Array.isArray(notebook.messages) && notebook.messages.length > 0)
    || (Array.isArray(notebook.artifacts) && notebook.artifacts.length > 0)
}
...
export function createResearchNotebookRepository(deps: ResearchNotebookRepositoryDependencies) {
  ...
  function getNotebookContentDocRef(uid: string, notebookId: string, contentDocId: string) {
    const db = deps.ensureFirestore()
    return doc(db, ...buildNotebookContentDocPath(uid, notebookId, contentDocId))
  }
  ...
  async function getNotebookContent(uid: string, notebookId: string): Promise<NotebookContentData & { hasMessagesDoc: boolean; hasArtifactsDoc: boolean }> {
    const [messagesSnapshot, artifactsSnapshot] = await Promise.all([
      deps.withFirestoreRetry(
        () => getDoc(getNotebookContentDocRef(uid, notebookId, NOTEBOOK_CONTENT_MESSAGES_DOC_ID)),
        'getNotebookContent.messages',
      ),
      deps.withFirestoreRetry(
        () => getDoc(getNotebookContentDocRef(uid, notebookId, NOTEBOOK_CONTENT_ARTIFACTS_DOC_ID)),
        'getNotebookContent.artifacts',
      ),
    ])

    const messagesData = messagesSnapshot.exists() ? messagesSnapshot.data() as NotebookContentData : null
    const artifactsData = artifactsSnapshot.exists() ? artifactsSnapshot.data() as NotebookContentData : null
    return {
      ...(Array.isArray(messagesData?.messages) ? { messages: messagesData.messages } : {}),
      ...(Array.isArray(artifactsData?.artifacts) ? { artifacts: artifactsData.artifacts } : {}),
      hasMessagesDoc: messagesSnapshot.exists(),
      hasArtifactsDoc: artifactsSnapshot.exists(),
    }
  }
  ...
  async function saveNotebookContent(
    uid: string,
    notebookId: string,
    payload: Partial<Pick<NotebookContentData, 'messages' | 'artifacts'>> & { migrated_from_notebook_doc_at?: string },
  ): Promise<void> {
    const updatedAt = new Date().toISOString()
    const writes: Array<Promise<unknown>> = []

    if (payload.messages !== undefined) {
      writes.push(deps.withFirestoreRetry(
        () => setDoc(
          getNotebookContentDocRef(uid, notebookId, NOTEBOOK_CONTENT_MESSAGES_DOC_ID),
          deps.stripUndefined({
            messages: payload.messages,
            updated_at: updatedAt,
            migrated_from_notebook_doc_at: payload.migrated_from_notebook_doc_at,
          }),
          { merge: true },
        ),
        'saveNotebookContent.messages',
      ))
    }
    ...
    if (payload.artifacts !== undefined) {
      writes.push(deps.withFirestoreRetry(
        () => setDoc(
          getNotebookContentDocRef(uid, notebookId, NOTEBOOK_CONTENT_ARTIFACTS_DOC_ID),
          deps.stripUndefined({
  ...
  async function backfillNotebookContentAcrossPlatform(opts?: {
    dryRun?: boolean
    maxNotebooks?: number
    chunkSize?: number
  }): Promise<NotebookContentBackfillReport> {
    const db = deps.ensureFirestore()
    const dryRun = Boolean(opts?.dryRun)
    const maxNotebooks = opts?.maxNotebooks && opts.maxNotebooks > 0 ? Math.floor(opts.maxNotebooks) : undefined
    const chunkSize = Math.max(50, Math.min(500, Math.floor(opts?.chunkSize ?? 200)))

    const report: NotebookContentBackfillReport = {
      scanned: 0,
      migrated: 0,
      already_dedicated: 0,
      empty_legacy: 0,
    ...
    while (true) {
      ...
      const notebooksSnapshot = await deps.withFirestoreRetry(
        () => getDocs(query(collectionGroup(db, 'research_notebooks'), ...constraints)),
        'backfillNotebookContentAcrossPlatform.notebooks',
      )
      if (notebooksSnapshot.empty) break
      ...
      for (const notebookDoc of notebooksSnapshot.docs) {
        ...
        try {
          ...
          const notebook = { id: notebookDoc.id, ...notebookDoc.data() } as ResearchNotebookData
          if (!hasNonEmptyLegacyContent(notebook)) {
            report.empty_legacy += 1
            continue
          }

          const content = await getNotebookContent(uid, notebookDoc.id)
          if (content.hasMessagesDoc && content.hasArtifactsDoc) {
            report.already_dedicated += 1
            continue
          }

          if (!dryRun) {
            await saveNotebookContent(uid, notebookDoc.id, {
              ...(!content.hasMessagesDoc ? { messages: notebook.messages ?? [] } : {}),
              ...(!content.hasArtifactsDoc ? { artifacts: notebook.artifacts ?? [] } : {}),
              migrated_from_notebook_doc_at: new Date().toISOString(),
            })
          }
          ...
          report.failed += 1
          console.warn('[Lexio] backfillNotebookContentAcrossPlatform: failed for notebook', notebookDoc.id, error)
        }
      }
  ...
  async function getResearchNotebook(uid: string, notebookId: string): Promise<ResearchNotebookData | null> {
    ...
    if (!snapshot.exists()) return null
    let notebook = { id: snapshot.id, ...snapshot.data() } as ResearchNotebookData
    ...
    try {
      ...
      } else if ((notebook.research_audits && notebook.research_audits.length > 0)
    ...
    try {
      const content = await getNotebookContent(effectiveUid, snapshot.id)
      notebook = {
        ...notebook,
        messages: content.messages ?? notebook.messages,
        artifacts: content.artifacts ?? notebook.artifacts,
      }

      const shouldSeedMessages = !content.hasMessagesDoc && Array.isArray(notebook.messages) && notebook.messages.length > 0
      const shouldSeedArtifacts = !content.hasArtifactsDoc && Array.isArray(notebook.artifacts) && notebook.artifacts.length > 0
      if (shouldSeedMessages || shouldSeedArtifacts) {
        await saveNotebookContent(effectiveUid, snapshot.id, {
          ...(shouldSeedMessages ? { messages: notebook.messages } : {}),
          ...(shouldSeedArtifacts ? { artifacts: notebook.artifacts } : {}),
          migrated_from_notebook_doc_at: new Date().toISOString(),
        })
      }
    } catch (error) {
      console.warn('[Lexio] getResearchNotebook: dedicated content unavailable, using notebook document fields.', error)
    }
  ...
  async function createResearchNotebook(
    ...
    const { artifacts } = fitArtifactsToFirestoreLimit(
      baseMeta.artifacts ?? [],
      estimateJsonBytes({ ...withMessages, artifacts: [] }),
    )
    const withContent = { ...withMessages, artifacts }
    const otherBytes = estimateJsonBytes(withContent)
    const { sources } = fitSourcesToFirestoreLimit(data.sources ?? [], otherBytes)

    const withSources = { ...withContent, sources }
    ...
    try {
      await saveNotebookContent(effectiveUid, docRef.id, {
        messages: data.messages ?? [],
        artifacts: data.artifacts ?? [],
        migrated_from_notebook_doc_at: now,
      })
    } catch (error) {
      console.warn('[Lexio] createResearchNotebook: failed to seed dedicated content store.', error)
    }
  ...
  async function updateResearchNotebook(
    ...
    const shouldSyncContent = hasNotebookContentPayload(rest)
    if (shouldSyncContent) {
      await saveNotebookContent(effectiveUid, normalizeFirestoreDocumentId(notebookId), {
        ...(rest.messages !== undefined ? { messages: rest.messages } : {}),
        ...(rest.artifacts !== undefined ? { artifacts: rest.artifacts } : {}),
      })
    }
    ...
    if (rootPayload.sources || rootPayload.messages || rootPayload.artifacts || rootPayload.llm_executions) {
      ...
      if (rootPayload.artifacts) {
        const merged = { ...existing, ...fittedPayload, updated_at: now }
        const { artifacts: _artifacts, ...mergedMeta } = merged
        const { artifacts } = fitArtifactsToFirestoreLimit(rootPayload.artifacts, estimateJsonBytes(mergedMeta))
        fittedPayload = { ...fittedPayload, artifacts }
      }
  ...
  async function deleteResearchNotebook(uid: string, notebookId: string): Promise<void> {
    ...
    for (const contentDocId of [NOTEBOOK_CONTENT_MESSAGES_DOC_ID, NOTEBOOK_CONTENT_ARTIFACTS_DOC_ID]) {
      try {
        await deps.withFirestoreRetry(
          () => deleteDoc(getNotebookContentDocRef(effectiveUid, normalizedNotebookId, contentDocId)),
          `deleteResearchNotebook.content.${contentDocId}`,
        )
      } catch {
        // Ignore missing/forbidden dedicated content docs; notebook deletion is the source of truth.
      }
    }
    try {
      await deps.withFirestoreRetry(
