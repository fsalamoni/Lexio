...
const DEFAULT_ANALISTA_BATCH_CONCURRENCY = 3
const MIN_ANALISTA_BATCH_CONCURRENCY = 1
const MAX_ANALISTA_BATCH_CONCURRENCY = 5
...
export async function analyzeNotebookAcervo(
  ...
  executions.push(createUsageExecutionRecord({
    ...
    provider_id: triageResult.provider_id ?? triageResult.operational?.providerId,
    provider_label: triageResult.provider_label ?? triageResult.operational?.providerLabel,
    requested_model: triageResult.operational?.requestedModel,
    resolved_model: triageResult.operational?.resolvedModel,
  ...
  executions.push(createUsageExecutionRecord({
    ...
    provider_id: buscadorResult.provider_id ?? buscadorResult.operational?.providerId,
    provider_label: buscadorResult.provider_label ?? buscadorResult.operational?.providerLabel,
    requested_model: buscadorResult.operational?.requestedModel,
    resolved_model: buscadorResult.operational?.resolvedModel,
  ...
  const processAnalistaBatch = async (batch: SelectedDocForAnalista[], batchIndex: number): Promise<void> => {
    ...
    try {
      ...
      executions.push(createUsageExecutionRecord({
        ...
        provider_id: analistaResult.provider_id ?? analistaResult.operational?.providerId,
        provider_label: analistaResult.provider_label ?? analistaResult.operational?.providerLabel,
        requested_model: analistaResult.operational?.requestedModel,
        resolved_model: analistaResult.operational?.resolvedModel,
  ...
  try {
    ...
    executions.push(createUsageExecutionRecord({
      ...
      provider_id: curadorResult.provider_id ?? curadorResult.operational?.providerId,
      provider_label: curadorResult.provider_label ?? curadorResult.operational?.providerLabel,
      requested_model: curadorResult.operational?.requestedModel,
      resolved_model: curadorResult.operational?.resolvedModel,
