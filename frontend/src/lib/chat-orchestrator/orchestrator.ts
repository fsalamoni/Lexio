...
function buildOrchestratorSystemPrompt(skills: Skill[], effort: string, expectedDeliverables: ChatExpectedDeliverable[]): string {
  ...
  return [
    ...
    '- Para tools que já suportam `approved: true` após confirmação (por exemplo `generate_image`, `generate_document`, `generate_studio_artifact`), chame a própria tool e deixe a skill pedir aprovação. Use `request_user_approval` direto apenas quando não houver tool com retomada embutida.',
