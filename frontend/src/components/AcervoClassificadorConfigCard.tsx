...
export default function AcervoClassificadorConfigCard() {
  ...
  const activeAgentDef = activeAgentKey
    ? ACERVO_CLASSIFICADOR_AGENT_DEFS.find(a => a.key === activeAgentKey)
    : null

  if (loading) {
    return <p className="text-gray-400 text-sm">Carregando configuração do Classificador de Acervo...</p>
  }
