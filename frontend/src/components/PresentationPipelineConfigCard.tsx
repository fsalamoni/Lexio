 ...
 * which LLM models the multi-agent presentation production pipeline uses.
 *
 * Follows the same design pattern as AcervoEmentaConfigCard
 * but targets the PRESENTATION_PIPELINE_AGENT_DEFS and presentation_pipeline_models Firestore key.
 */
...
const CAPABILITY_WARNINGS: Record<string, string> = {
  video: '⚠️ Requer modelo de vídeo',
  image: '⚠️ Requer modelo de imagem',
  audio: '⚠️ Requer modelo de áudio',
}
  ...
  if (loading) {
    return (
      <div className="bg-white rounded-xl border p-6 mb-6">
        <p className="text-gray-400 text-sm">Carregando configuração do Pipeline de Apresentação...</p>
      </div>
    )
  }
      ...
      <div className="bg-white rounded-xl border mb-6 overflow-hidden">
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-semibold text-gray-900">Pipeline de Apresentação</h2>
              <p className="text-sm text-gray-500">Configure os modelos LLM para a produção multi-agente de apresentações profissionais</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            {hasNonDefaults && (
        ...
        {expanded && (
          <div className="px-6 pb-6">
            ...
            <div className="flex items-center gap-2 mb-4 pb-3 border-b">
              ...
              <span className="text-sm font-semibold text-gray-700">Pipeline de Produção de Apresentação</span>
              <span className="text-xs text-gray-400 ml-auto">5 agentes · pipeline de produção de apresentação</span>
            </div>

            {/* Agent cards */}
            <div className="space-y-0">
              {PRESENTATION_PIPELINE_AGENT_DEFS.map((agent) => {
                ...
                const capWarning   = agent.requiredCapability && agent.requiredCapability !== 'text'
                  ? CAPABILITY_WARNINGS[agent.requiredCapability]
                  : null
                    ...
                    <div className={`relative rounded-lg border p-4 transition-colors ${
                      ...
                      <div className="flex items-start gap-3">
                        <div className={`w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 ${
                          ...
                          <Icon className={`w-4 h-4 ${!isDefault ? 'text-amber-600' : 'text-gray-600'}`} />
                        </div>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <button
                            ...
                            className={`w-full flex items-center justify-between gap-2 px-3 py-2 rounded-lg border text-sm transition-colors hover:border-amber-300 hover:bg-amber-50/50 ${
                              !isDefault
                          ...
                          {agent.bestModelNote && (
                            <p className="text-[10px] mt-1 px-2 py-0.5 rounded bg-amber-50 text-amber-600 inline-block font-medium">
                              💡 {agent.bestModelNote}
                            </p>
                          )}
            ...
            <div className="mt-4 p-3 bg-amber-50 border border-amber-200 rounded-lg">
              <p className="text-xs text-amber-800">
                📊 <strong>Pipeline de Apresentação:</strong> A produção de apresentação usa 5 agentes especializados — do planejamento e conteúdo até design e ilustração. O Ilustrador pode usar um modelo de geração de imagem para criar visuais de alta qualidade, mas modelos de texto também funcionam produzindo descrições detalhadas.
              </p>
            </div>
            ...
            <div className="flex items-center gap-3 mt-4 pt-4 border-t">
              <button
                ...
                className="inline-flex items-center gap-2 px-4 py-2 text-sm bg-amber-600 text-white rounded-lg hover:bg-amber-700 disabled:opacity-40 disabled:cursor-not-allowed font-medium transition-colors"
              >
                {saving
