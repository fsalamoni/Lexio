...
- ✅ Quinta fatia estrutural aplicada: definições de agentes foram separadas por pipeline em `frontend/src/lib/pipelines/agent-definitions/`; `model-config.ts` permanece como fachada compatível e dono da persistência/validação user-scoped.
...
- ✅ Oitava fatia estrutural aplicada: CRUD de documentos e persistência `documento_caderno` foram extraídos para `frontend/src/lib/modules/documents/repository.ts`; `firestore-service.ts` agora instancia o repository e preserva as exports públicas (`createDocument`, `getDocument`, `listDocuments`, `updateDocument`, `deleteDocument`, `saveNotebookDocumentToDocuments`) como fachada compatível.
- ✅ Validação local pós-documents-repository concluída: guardrail arquitetural (`314` arquivos fonte), typecheck, testes focados de Firestore/Documents (**4 arquivos**, **37 testes**), suíte completa frontend (**71/71 arquivos**, **515/515 testes**) e build com `VITE_FIRESTORE_DATABASE_ID=lexio-prod` verdes.
- ✅ Nona fatia estrutural aplicada: CRUD/listagem/stats do Banco de Teses foram extraídos para `frontend/src/lib/modules/theses/repository.ts`; `firestore-service.ts` agora instancia o repository e preserva as exports públicas (`listTheses`, `createThesis`, `updateThesis`, `deleteThesis`, `getThesisStats`) como fachada compatível.
- ✅ Validação local pós-theses-repository concluída: guardrail arquitetural (`316` arquivos fonte), typecheck, testes focados de Firestore/Teses/consumidores (**4 arquivos**, **45 testes**), suíte completa frontend (**71/71 arquivos**, **515/515 testes**) e build com `VITE_FIRESTORE_DATABASE_ID=lexio-prod` verdes.
- ✅ Décima fatia estrutural aplicada: CRUD, leituras indexadas, ementas/tags, conversão de texto para JSON compacto e marcações de análise de Acervo foram extraídos para `frontend/src/lib/modules/acervo/repository.ts`; `firestore-service.ts` agora preserva as exports públicas (`listAcervoDocuments`, `createAcervoDocument`, `deleteAcervoDocument`, `getAllAcervoDocumentsForSearch`, `updateAcervoEmenta`, `updateAcervoTags`, `updateAcervoTextContent`, `convertAcervoToJson`, `getAcervoDocsWithoutTags`, `getAcervoDocsWithoutEmenta`, `getAcervoContext`, `markAcervoDocumentsAnalyzed`, `getAcervoAnalysisStatus`) como fachada compatível.
- ✅ Validação local pós-acervo-repository concluída: manifest JSON, guardrail arquitetural (`318` arquivos fonte), typecheck, testes focados de guardrails/Acervo/consumidores (**3 arquivos**, **10 testes**), teste Firestore retry-heavy (**1 arquivo**, **28 testes**), suíte completa frontend (**71/71 arquivos**, **515/515 testes**) e build com `VITE_FIRESTORE_DATABASE_ID=lexio-prod` verdes.
- ✅ Décima primeira fatia estrutural aplicada: CRUD, segurança de tamanho do documento, memória dedicada de buscas e backfill de Research Notebook foram extraídos para `frontend/src/lib/modules/notebook/repository.ts`; `firestore-service.ts` agora preserva as exports públicas (`backfillNotebookSearchMemoryAcrossPlatform`, `listResearchNotebooks`, `getResearchNotebook`, `createResearchNotebook`, `updateResearchNotebook`, `deleteResearchNotebook`) como fachada compatível.
- ✅ Validação local pós-research-notebook-repository concluída: manifest JSON, guardrail arquitetural (`319` arquivos fonte), typecheck, testes focados de Firestore/Research Notebook (**4 arquivos**, **42 testes**), suíte completa frontend (**71/71 arquivos**, **515/515 testes**) e build com `VITE_FIRESTORE_DATABASE_ID=lexio-prod` verdes.
- ✅ Décima segunda fatia estrutural aplicada: sessões de análise de teses (`listThesisAnalysisSessions`, `saveThesisAnalysisSession`, `getLastThesisAnalysisSession`) foram extraídas para `frontend/src/lib/modules/theses/repository.ts`; `firestore-service.ts` agora preserva essas exports públicas como fachada compatível para ThesisAnalysisCard, dashboard, custos e analytics locais.
- ✅ Validação local pós-thesis-analysis-sessions concluída: manifest JSON, guardrail arquitetural (`319` arquivos fonte), typecheck, testes focados de Firestore/Teses (**3 arquivos**, **40 testes**), suíte completa frontend (**71/71 arquivos**, **517/517 testes**) e build com `VITE_FIRESTORE_DATABASE_ID=lexio-prod` verdes.
- ✅ Décima terceira fatia estrutural aplicada: Profile/Onboarding (`getProfile`, `saveProfile`, `completeOnboarding`, `getWizardData`) foi extraído para `frontend/src/lib/modules/profile/repository.ts`; `firestore-service.ts` preserva essas exports públicas como fachada compatível para Profile, Onboarding, NewDocument e laboratórios V2.
- ✅ Validação local pós-profile-repository concluída: manifest JSON, guardrail arquitetural (`321` arquivos fonte), typecheck, testes focados de Firestore/Profile/consumidores (**3 arquivos**, **40 testes**), suíte completa frontend (**71/71 arquivos**, **521/521 testes**), build com `VITE_FIRESTORE_DATABASE_ID=lexio-prod`, `git diff --check` e Problems verdes.
- ✅ Décima quarta fatia estrutural aplicada: Settings/Migration (`getSettings`, `saveSettings`, `getUserSettings`, `saveUserSettings`, `ensureUserSettingsMigrated`) foi extraída para `frontend/src/lib/modules/settings/repository.ts`; `firestore-service.ts` preserva essas exports públicas como fachada compatível para settings-store, model-config, model-catalog, provider credentials e health checks.
- ✅ Validação local pós-settings-repository concluída: manifest JSON, guardrail arquitetural (`323` arquivos fonte), typecheck, testes focados de Firestore/Settings/consumidores (**4 arquivos**, **59 testes**), suíte completa frontend (**71/71 arquivos**, **524/524 testes**), build com `VITE_FIRESTORE_DATABASE_ID=lexio-prod` e `git diff --check` verdes.
- ✅ Décima quinta fatia estrutural aplicada: Admin Taxonomy (`getDocumentTypes`, `getLegalAreas`, sanitizers, `load/saveAdmin*`, filtros por perfil e `getRequestFields`) foi extraída para `frontend/src/lib/modules/admin-taxonomy/repository.ts`; `firestore-service.ts` preserva as exports públicas como fachada compatível para AdminPanel, Upload, NewDocument, geração e orquestrador V3.
- ✅ Validação local pós-admin-taxonomy-repository concluída: guardrail arquitetural (`325` arquivos fonte), typecheck, testes focados de Firestore/Admin Taxonomy/consumidores (**5 arquivos**, **62 testes**), suíte completa frontend (**71/71 arquivos**, **530/530 testes**) e build com `VITE_FIRESTORE_DATABASE_ID=lexio-prod` verdes.
- ✅ Décima sexta fatia estrutural aplicada: Dashboard Stats/Cost (`getStats`, `getDailyStats`, `getByTypeStats`, `getRecentDocuments`, `getDashboardSnapshot`, `getCostBreakdown`) foi extraída para `frontend/src/lib/modules/dashboard/repository.ts`; os builders puros compartilhados agora vivem em `frontend/src/lib/modules/dashboard/metrics.ts` e `firestore-service.ts` preserva as exports públicas como fachada compatível para Dashboard, AdminPanel e CostTokensPage.
...
- 🔄 Próximo checkpoint: manifest JSON, `git diff --check`, Problems, commit/push incremental e nova verificação remota do PR #143. Resta exatamente **1 fatia** planejada em `firestore-service.ts`: Chat.
...
- ✅ `scripts/firebase-cloud-sync.mjs` passou a aceitar `--database-id`, registrar `databaseId` no manifesto, preservar `rawFields` do Firestore REST e gerar checksums SHA-256 com verificação opcional por `--verify-manifest`.
- ✅ Scripts operacionais seguros criados: auditoria/classificação read-only (`lexio-firestore-audit.mjs`), migração shadow dry-run por padrão (`lexio-firestore-migrate-shadow.mjs`) e validação de paridade entre snapshots (`lexio-firestore-validate-shadow.mjs`).
- ✅ `frontend/src/lib/firebase.ts` ganhou roteamento por `VITE_FIRESTORE_DATABASE_ID`, mantendo `(default)` como fallback e sem alterar produção.
...
- ✅ `NewDocument.tsx` e `DocumentDetail.tsx` foram alinhados ao contrato canônico (`deriveExecutionState` + `normalizeProgressForExecution`) para evitar distorções de percentual em wrappers de UI/tarefa.
- ✅ `notebook-audio-pipeline.ts` recebeu quick win de latência com aumento do teto de concorrência adaptativa de TTS (`MAX 3 -> 5`) e telemetria explícita de lote/perfil runtime no progresso.
- ✅ `generation-service.ts` foi otimizado com warm-up de ementas por orçamento de tempo (janela curta não-bloqueante), reduzindo bloqueio da trilha documental sem perda de fallback/qualidade.
...
**Atualização incremental (2026-05-05 — Wave 40: latência documental + canários/runtime + caches user-scoped, subonda 2 fechada em `main`):**
...
- ✅ Closeout operacional concluído nesta rodada: o smoke autenticado local foi destravado com `VITE_FORCE_DEMO_MODE=true` + credenciais fixas de smoke, o payload funcional foi fechado no commit `162224f` e os deploys ficaram verdes com Firebase (`25405834740`) e `release-web.yml` para GitHub Pages (`25405834580`) em sucesso.
- 🔄 Próximo bloco: abrir nova frente funcional sem pendências operacionais desta subonda; a revalidação com Firebase Auth real permanece opcional como verificação pós-closeout.
...
- ✅ `firestore-service.ts` passou a calcular confiança operacional por função e thresholds preditivos adaptativos (`resolveFunctionPredictiveThresholds`, `computeFunctionRolloutConfidence`), refinando risco/recomendação para reduzir falsos positivos sem cegar sinais críticos.
- ✅ `PlatformAdminPanel.tsx` integrou o bloco executivo Wave 39 com distribuição de confiança, contagem de alertas preditivos, watchlist de baixa confiança e painel de alinhamento com a demonstração dos agentes trabalhando.
- ✅ Validação local completa desta wave: `npm run typecheck`, `npm run test -- --run` (**38/38 arquivos, 299/299 testes**), `npm run build`, `functions npm run build` e `pytest` (**2203/2203**), sem regressões.
- ✅ Trilha operacional da wave concluída: commit/push do `main` (`a2ed246`) e one-shot release (`release-web.yml` run `24939740593`) em `success`, com quality gates (`73031338133`, `73031338134`, `73031338135`, `73031338137`, `73031338143`), deploy Firebase (`73031401264`), Pages build/deploy (`73031401290`, `73031468718`), release summary (`73031560811`) e redesign V2 em skip por input (`73031401347`).
- 🔄 Próximo bloco: monitorar precisão dos alertas preditivos por banda de confiança e recalibrar thresholds adaptativos por criticidade para manter sensibilidade em risco crítico com menor ruído operacional.

**Atualização incremental (2026-04-25 — Wave 38: política progressiva por criticidade + guardrails preditivos de drift):**
- ✅ `firestore-types.ts` foi ampliado com os contratos da política progressiva (`PlatformFunctionRolloutRecommendation`, `PlatformFunctionRolloutRiskLevel`, `PlatformFunctionRolloutGuardrails`, `PlatformFunctionRolloutPolicyRow`, `PlatformFunctionRolloutPolicyPlan`) para leitura tipada de risco/recomendação por função.
- ✅ `firestore-service.ts` passou a expor `getPlatformFunctionRolloutPolicyPlan(...)`, calculando tendência diária por função, streaks de aderência, risco por criticidade e recomendação progressiva (`tighten_now`, `tighten_guarded`, `hold`, `relax_guarded`) com guardrails operacionais.
- ✅ `PlatformAdminPanel.tsx` integrou um bloco executivo Wave 38 com resumo por recomendação, tabela de risco/recomendação por função, alertas preditivos de drift combinado (`retry + waiting I/O`) e recomendações acionáveis, preservando a demonstração dos agentes trabalhando.
- ✅ Validação local completa desta wave: `npm run typecheck`, `npm run test -- --run` (**38/38 arquivos, 299/299 testes**), `npm run build`, `functions npm run build` e `pytest` (**2203/2203**), sem regressões.
- ✅ Trilha operacional da wave concluída: commit/push do `main` (`f5cbf57`) e one-shot release (`release-web.yml` run `24933092299`) em `success`, com quality gates (`73013994216`, `73013994220`, `73013994224`, `73013994226`, `73013994229`), deploy Firebase (`73014056225`), Pages build/deploy (`73014056272`, `73014131480`), release summary (`73014216569`) e redesign V2 em skip por input (`73014056331`).
- 🔄 Próximo bloco: consolidar feedback de produção para calibrar limites de guardrail por criticidade e reduzir falsos positivos no gatilho preditivo de drift combinado.

**Atualização incremental (2026-04-25 — Wave 37: aderência diária live vs alvo por função + rollout assistido):**
- ✅ `firestore-types.ts` foi ampliado com os contratos de aderência diária (`PlatformFunctionTargetAdherenceStatus`, `PlatformFunctionTargetAdherenceRow`, `PlatformFunctionTargetAdherenceDailyPoint`) para monitoramento tipado de status live versus alvo por função.
- ✅ `firestore-service.ts` passou a expor `getPlatformFunctionTargetAdherenceDaily(...)`, agregando por dia as funções monitoradas com cálculo de pressão live/alvo, cobertura de plano e classificação `above_target`/`aligned`/`below_target`.
- ✅ `PlatformAdminPanel.tsx` integrou um novo bloco executivo de aderência diária com cards de estabilidade/cobertura, tabela de funções (retry/fallback/waiting I/O live/alvo), tendência de 7 dias e recomendações de rollout assistido, preservando a demonstração dos agentes trabalhando.
- ✅ Validação local completa desta wave: `npm run typecheck`, `npm run test -- --run` (**38/38 arquivos, 299/299 testes**), `npm run build`, `functions npm run build` e `pytest -q` (**2203/2203**), sem regressões.
- ✅ Trilha operacional da wave concluída: commit/push do `main` (`db87300`) e one-shot release (`release-web.yml` run `24930689755`) em `success`, com quality gates + Firebase + Pages e redesign V2 em skip por input.
- 🔄 Próximo bloco: transformar as recomendações de aderência diária em política progressiva de rollout por criticidade (guardrails de teto por função e confirmação de estabilidade por janela).

**Atualização incremental (2026-04-25 — Wave 36: calibração adaptativa por função + demonstração multiagente alinhada a alvos):**
- ✅ `firestore-types.ts` foi ampliado com contratos de calibração por função (`PlatformFunctionCalibrationRow`, `PlatformFunctionCalibrationAction`, `PlatformFunctionCalibrationPriority`) para leitura tipada de alvo atual/próxima janela.
- ✅ `firestore-service.ts` passou a expor `getPlatformFunctionCalibrationPlan(...)`, gerando plano adaptativo por função com score de risco, prioridade, ação recomendada (`tighten`/`maintain`/`relax`) e alvos para retry/fallback/waiting I/O.
- ✅ `PlatformAdminPanel.tsx` integrou novo bloco executivo de calibração adaptativa por função (cards de plano, tabela de alvos por função, recomendações automáticas e leitura de status live versus alvo), preservando e reforçando a demonstração dos agentes trabalhando.
- ✅ Validação local completa desta wave: `npm run typecheck`, `npm run test -- --run` (**38/38 arquivos, 299/299 testes**), `npm run build`, `functions npm run build` e `pytest -q` (**2203/2203**), sem regressões.
- ✅ Trilha operacional da wave concluída: commit/push do `main` (`cfdb2ac`) e one-shot release (`release-web.yml` run `24919950308`) em `success`, com quality gates + Firebase + Pages e redesign V2 em skip por input.
- 🔄 Próximo bloco: monitorar aderência dos sinais live versus alvo por função em produção e calibrar rollout assistido de ajustes sem regressão de UX/custo.

**Atualização incremental (2026-04-25 — Wave 35: comparativo diário por função + tuning fino orientado por janela):**
- ✅ `firestore-types.ts` foi ampliado com o contrato `PlatformFunctionWindowComparisonRow`, permitindo comparação consistente de chamadas/custo/latência/retry/fallback/waiting I/O por função entre janelas consecutivas.
- ✅ `firestore-service.ts` passou a expor `getPlatformFunctionWindowComparison(...)`, reutilizando o extrator operacional existente e agregando risco por função na janela atual versus anterior.
- ✅ `PlatformAdminPanel.tsx` ganhou nova seção executiva de comparativo por função (cards de delta, tabela detalhada e recomendações automáticas por hotspot), mantendo e reforçando a demonstração dos agentes trabalhando com telemetria real.
...
- ✅ Trilha operacional da wave concluída: commit/push do `main` (`cf5b673`) e one-shot release (`release-web.yml` run `24919036006`) em `success`, com quality gates + Firebase + Pages e redesign V2 em skip por input.
- 🔄 Próximo bloco: monitorar impacto em produção do comparativo diário por função (especialmente drifts em `waiting_io` e `retrying`) e calibrar thresholds por função sem regressão de UX.

**Atualização incremental (2026-04-24 — Wave 34: comparativo diário por execution_state + janela atual vs anterior):**
- ✅ `firestore-types.ts` foi ampliado com contratos dedicados para monitoramento diário por estado (`PlatformExecutionStateDailyPoint`) e comparação de janelas (`PlatformExecutionStateWindowComparisonRow`).
...
- ✅ Trilha operacional da wave concluída: commit/push funcional do `main` (`7b2d321`), one-shot release (`release-web.yml` run `24859770023`) em `success` com quality gates + Firebase + Pages, e sincronização final de governança/index/cache com pacote de handoff cross-platform.
...
- ✅ Próximo bloco da Wave 29 concluído na Wave 30: emissão explícita de `executionState` expandida para pipelines auxiliares e wrappers de tarefa.
...
- ✅ Trilha operacional completa da wave concluída: pull/rebase (`Already up to date`), commit/push do `main` (`681c767`) e execução one-shot de release (`release-web.yml` run `24854808367`) com quality gates + Firebase + Pages + summary em `success` (redesign V2 skipped por input).
- 🔄 Próximo bloco: medir impacto de latência/custo em produção após a expansão do contrato explícito de `executionState`, consolidando tuning por pipeline auxiliar sem regressão de UX.
...
- ✅ Closeout operacional da subonda concluído: o smoke autenticado local foi finalizado em modo smoke/demo, o payload funcional `162224f` foi publicado em `main` e os deploys fecharam em sucesso no Firebase (`25405834740`) e no one-shot de Pages (`25405834580`); a revalidação em Firebase real fica como verificação opcional pós-closeout
