...
## Status Geral: Etapas 1-379 IMPLEMENTADAS · Wave 42 firestore-service extraction phase completa e corte produtivo `lexio-prod` concluído

### Etapas 351-379 — Wave 42 (Core/Módulos Foundation) ⚠️
- **Arquivos**: `scripts/lexio-architecture-guardrails.mjs`, `.github/workflows/test.yml`, `frontend/package.json`, `frontend/src/lib/modules/dashboard/index.ts`, `frontend/src/lib/modules/dashboard/dashboard-data.ts`, `frontend/src/lib/modules/dashboard/dashboard-v2.ts`, `frontend/src/lib/modules/dashboard/metrics.ts`, `frontend/src/lib/modules/dashboard/repository.ts`, `frontend/src/lib/modules/dashboard/dashboard-data.test.ts`, `frontend/src/lib/modules/dashboard/dashboard-v2.test.ts`, `frontend/src/lib/dashboard-data.ts`, `frontend/src/lib/dashboard-v2.ts`, `frontend/src/lib/modules/notebook/index.ts`, `frontend/src/lib/modules/notebook/notebook-pipeline-progress.ts`, `frontend/src/lib/modules/notebook/notebook-constants.ts`, `frontend/src/lib/modules/notebook/repository.ts`, `frontend/src/lib/notebook-pipeline-progress.ts`, `frontend/src/lib/notebook-constants.ts`, `frontend/src/lib/modules/documents/index.ts`, `frontend/src/lib/modules/documents/prompts.ts`, `frontend/src/lib/modules/documents/prompts.test.ts`, `frontend/src/lib/modules/documents/acervo.ts`, `frontend/src/lib/modules/documents/acervo.test.ts`, `frontend/src/lib/modules/documents/json.ts`, `frontend/src/lib/modules/documents/repository.ts`, `frontend/src/lib/modules/theses/index.ts`, `frontend/src/lib/modules/theses/repository.ts`, `frontend/src/lib/modules/acervo/index.ts`, `frontend/src/lib/modules/acervo/repository.ts`, `frontend/src/lib/modules/profile/index.ts`, `frontend/src/lib/modules/profile/repository.ts`, `frontend/src/lib/modules/settings/index.ts`, `frontend/src/lib/modules/settings/repository.ts`, `frontend/src/lib/modules/admin-taxonomy/index.ts`, `frontend/src/lib/modules/admin-taxonomy/repository.ts`, `frontend/src/lib/modules/chat/index.ts`, `frontend/src/lib/modules/chat/repository.ts`, `frontend/src/lib/pipelines/agent-definitions/index.ts`, `frontend/src/lib/pipelines/agent-definitions/document-v2.ts`, `frontend/src/lib/pipelines/agent-definitions/thesis-analyst.ts`, `frontend/src/lib/pipelines/agent-definitions/context-detail.ts`, `frontend/src/lib/pipelines/agent-definitions/acervo-classificador.ts`, `frontend/src/lib/pipelines/agent-definitions/acervo-ementa.ts`, `frontend/src/lib/pipelines/agent-definitions/research-notebook.ts`, `frontend/src/lib/pipelines/agent-definitions/notebook-acervo.ts`, `frontend/src/lib/pipelines/agent-definitions/video.ts`, `frontend/src/lib/pipelines/agent-definitions/audio.ts`, `frontend/src/lib/pipelines/agent-definitions/presentation.ts`, `frontend/src/lib/pipelines/agent-definitions/document-v3.ts`, `frontend/src/lib/pipelines/agent-definitions/chat-orchestrator.ts`, `frontend/src/lib/model-config.ts`, `frontend/src/lib/agent-config-coverage.test.ts`, `frontend/src/lib/platform-analytics.ts`, `frontend/src/lib/core/firestore/index.ts`, `frontend/src/lib/core/firestore/paths.ts`, `frontend/src/lib/core/firestore/paths.test.ts`, `frontend/src/lib/firestore-service.ts`, `frontend/src/lib/firestore-service-auth-guardrails.test.ts`, `frontend/src/lib/firestore-service.test.ts`, `frontend/src/lib/generation-service.ts`, `frontend/src/lib/document-v3-orchestrator.ts`, `frontend/src/lib/pipeline-step-execution-state.test.ts`, `frontend/src/pages/Dashboard.tsx`, `frontend/src/pages/labs/DashboardV2.tsx`, `frontend/src/pages/ResearchNotebook.tsx`, `frontend/src/pages/labs/ResearchNotebookV2.tsx`, `frontend/src/pages/notebook/constants.ts`, `docs/architecture/core-and-modules.md`, `docs/architecture/domain-map.md`, `docs/architecture/module-contracts.md`, `docs/architecture/dependency-rules.md`, `docs/architecture/ai-development-guide.md`, `docs/guides/adding-module.md`, `docs/guides/refactoring-module.md`, `docs/guides/testing-modules.md`, `docs/architecture/adr/0001-core-modules-incremental.md`, `docs/PLANO.md`, `docs/MANIFEST.json`, `NOTEBOOK_IMPLEMENTATION_STATUS.md`
...
- Etapa 356: validação local e remota do primeiro commit concluída: PR #143 com guardrails, frontend quality, Python, functions, Firebase secrets, preview deploy e Agent em sucesso.
- Etapa 357: primeiro módulo de produto extraído para `frontend/src/lib/modules/dashboard/`, com `index.ts` público, testes próximos ao módulo e facades compatíveis em `frontend/src/lib/dashboard-data.ts` e `frontend/src/lib/dashboard-v2.ts`.
- Etapa 358: validação completa pós-dashboard concluída: guardrail, typecheck, testes focados, suíte completa frontend e build com `VITE_FIRESTORE_DATABASE_ID=lexio-prod` verdes; commit `510f3b9` enviado ao PR #143.
- Etapa 359: segundo módulo de produto iniciado para `frontend/src/lib/modules/notebook/`, concentrando contratos de progresso e constantes puras; consumidores diretos passaram a importar do entrypoint público do módulo.
- Etapa 360: validação completa pós-notebook concluída com guardrail, typecheck, testes focados, suíte completa frontend e build com `VITE_FIRESTORE_DATABASE_ID=lexio-prod` verdes; commit `7978792` enviado ao PR #143 e checks remotos verdes.
- Etapa 361: terceiro módulo de produto iniciado para `frontend/src/lib/modules/documents/`, concentrando metadados de tipo/área, helper de perfil e prompt de usuário do Pesquisador; `generation-service.ts` mantém exports compatíveis e `document-v3-orchestrator.ts` consome o entrypoint modular.
- Etapa 362: validação completa pós-documents concluída localmente: guardrail arquitetural (`293` arquivos fonte), typecheck, testes focados, suíte completa frontend (**69/69 arquivos**, **508/508 testes**) e build com `VITE_FIRESTORE_DATABASE_ID=lexio-prod` verdes. Commit/push incremental e nova verificação remota do PR #143 são o fechamento operacional desta etapa.
- Etapa 363: helpers de acervo documental extraídos para `frontend/src/lib/modules/documents/`, incluindo prompts `buildAcervo*`, seleção/prefiltro de documentos, extração de keywords e helper JSON compartilhado; `generation-service.ts` permanece como orquestrador e mantém compatibilidade pública para `selectAcervoDocsForBuscador`.
- Etapa 364: validação completa pós-acervo concluída localmente: manifest JSON, guardrail arquitetural (`296` arquivos fonte), typecheck, testes focados, suíte completa frontend (**70/70 arquivos**, **511/511 testes**) e build com `VITE_FIRESTORE_DATABASE_ID=lexio-prod` verdes. Commit/push incremental e nova verificação remota do PR #143 são o fechamento operacional desta etapa.
- Etapa 365: definições de agentes separadas por pipeline em `frontend/src/lib/pipelines/agent-definitions/`, cobrindo documento v2/v3, teses, context detail, acervo, notebook, mídia, apresentação e chat; `model-config.ts` permanece como fachada pública e dono da persistência/validação user-scoped. Validação local concluída: manifest JSON, guardrail arquitetural (`309` arquivos fonte), typecheck, teste focado de cobertura, suíte completa frontend (**70/70 arquivos**, **512/512 testes**) e build com `VITE_FIRESTORE_DATABASE_ID=lexio-prod` verdes.
- Etapa 366: analytics de plataforma extraído de `firestore-service.ts` para `frontend/src/lib/platform-analytics.ts`, concentrando cache, `collectionGroup` reads, agregações de custo/uso, janelas por `execution_state`, calibração por função, aderência diária e plano de rollout progressivo; `firestore-service.ts` permanece como fachada compatível para os imports públicos do painel/admin custos. Validação local concluída: manifest JSON, guardrail arquitetural (`310` arquivos fonte), typecheck, testes focados de custo/Firestore (**2 arquivos**, **31 testes**), teste estrutural de auth/retry, suíte completa frontend (**70/70 arquivos**, **512/512 testes**) e build com `VITE_FIRESTORE_DATABASE_ID=lexio-prod` verdes.
- Etapa 367: helpers puros de paths/references do Firestore criados em `frontend/src/lib/core/firestore/`, com entrypoint público, cobertura própria e migração inicial de `firestore-service.ts`/`platform-analytics.ts` para `normalizeFirestoreDocumentId`, `getRefUserId`, `getRefNotebookIdFromSearchMemoryPath`, `NOTEBOOK_SEARCH_MEMORY_DOC_ID` e builders tipados de paths de notebooks/memória. Validação local concluída: guardrail arquitetural (`313` arquivos fonte), typecheck, testes focados (**3 arquivos**, **35 testes**), suíte completa frontend (**71/71 arquivos**, **515/515 testes**) e build com `VITE_FIRESTORE_DATABASE_ID=lexio-prod` verdes.
- Etapa 368: repository Firestore de Documents extraído para `frontend/src/lib/modules/documents/repository.ts`, concentrando CRUD de documentos, listagem com fallback client-side, atualização/remoção e persistência `documento_caderno`; `firestore-service.ts` permanece como fachada compatível para imports públicos usados por páginas, admin, sidebar e notebook. Validação local concluída: guardrail arquitetural (`314` arquivos fonte), typecheck, testes focados (**4 arquivos**, **37 testes**), suíte completa frontend (**71/71 arquivos**, **515/515 testes**) e build com `VITE_FIRESTORE_DATABASE_ID=lexio-prod` verdes.
- Etapa 369: repository Firestore do Banco de Teses extraído para `frontend/src/lib/modules/theses/repository.ts`, concentrando listagem com fallback client-side, CRUD e stats de teses; `firestore-service.ts` permanece como fachada compatível para imports públicos usados por ThesisBank, análise de teses, geração e agentes V3. Validação local concluída: guardrail arquitetural (`316` arquivos fonte), typecheck, testes focados (**4 arquivos**, **45 testes**), suíte completa frontend (**71/71 arquivos**, **515/515 testes**) e build com `VITE_FIRESTORE_DATABASE_ID=lexio-prod` verdes.
- Etapa 370: repository Firestore de Acervo extraído para `frontend/src/lib/modules/acervo/repository.ts`, concentrando CRUD, leituras indexadas/fallbacks, geração de ementa/tags, atualização/conversão de texto em JSON compacto e status de análise para teses; `firestore-service.ts` permanece como fachada compatível para imports públicos usados por upload, geração, análise de acervo e teses. Validação local concluída: manifest JSON, guardrail arquitetural (`318` arquivos fonte), typecheck, testes focados de guardrails/Acervo/consumidores (**3 arquivos**, **10 testes**), teste Firestore retry-heavy (**1 arquivo**, **28 testes**), suíte completa frontend (**71/71 arquivos**, **515/515 testes**) e build com `VITE_FIRESTORE_DATABASE_ID=lexio-prod` verdes.
- Etapa 371: repository Firestore de Research Notebook extraído para `frontend/src/lib/modules/notebook/repository.ts`, concentrando CRUD dos cadernos, segurança de tamanho do documento, memória dedicada de buscas e backfill platform-wide da memória; `firestore-service.ts` permanece como fachada compatível para imports públicos usados por ResearchNotebook, ResearchNotebookV2, custo operacional e rotinas de migração. Validação local concluída: manifest JSON, guardrail arquitetural (`319` arquivos fonte), typecheck, testes focados de Firestore/Research Notebook (**4 arquivos**, **42 testes**), suíte completa frontend (**71/71 arquivos**, **515/515 testes**) e build com `VITE_FIRESTORE_DATABASE_ID=lexio-prod` verdes.
- Etapa 372: sessões de análise de teses extraídas para `frontend/src/lib/modules/theses/repository.ts`, concentrando listagem, leitura da sessão mais recente e persistência; `firestore-service.ts` permanece como fachada compatível para imports públicos usados por ThesisAnalysisCard, dashboard e métricas de custo. Validação local concluída: manifest JSON, guardrail arquitetural (`319` arquivos fonte), typecheck, testes focados de Firestore/Teses (**3 arquivos**, **40 testes**), suíte completa frontend (**71/71 arquivos**, **517/517 testes**) e build com `VITE_FIRESTORE_DATABASE_ID=lexio-prod` verdes.
- Etapa 373: repository Firestore de Profile/Onboarding extraído para `frontend/src/lib/modules/profile/repository.ts`, concentrando leitura/salvamento de perfil, conclusão de onboarding e montagem dos passos canônicos do wizard; `firestore-service.ts` permanece como fachada compatível para imports públicos usados por Profile, Onboarding, NewDocument e laboratórios V2. Validação local concluída: manifest JSON, guardrail arquitetural (`321` arquivos fonte), typecheck, testes focados de Firestore/Profile/consumidores (**3 arquivos**, **40 testes**), suíte completa frontend (**71/71 arquivos**, **521/521 testes**), build com `VITE_FIRESTORE_DATABASE_ID=lexio-prod`, `git diff --check` e Problems verdes. Restam exatamente 4 repositories planejados nesta fase: Settings/Migration, Admin Taxonomy, Dashboard Stats/Cost e Chat.
- Etapa 374: repository Firestore de Settings/Migration extraído para `frontend/src/lib/modules/settings/repository.ts`, concentrando leitura/escrita de `/settings/platform`, leitura/escrita de `/users/{uid}/settings/preferences` e migração legada para preferências user-scoped; `firestore-service.ts` permanece como fachada compatível para settings-store, model-config, model-catalog, provider credentials e health checks. Validação local concluída: manifest JSON, guardrail arquitetural (`323` arquivos fonte), typecheck, testes focados de Firestore/Settings/consumidores (**4 arquivos**, **59 testes**), suíte completa frontend (**71/71 arquivos**, **524/524 testes**), build com `VITE_FIRESTORE_DATABASE_ID=lexio-prod` e `git diff --check` verdes. Restam exatamente 3 repositories planejados nesta fase: Admin Taxonomy, Dashboard Stats/Cost e Chat.
- Etapa 375: repository de Admin Taxonomy extraído para `frontend/src/lib/modules/admin-taxonomy/repository.ts`, concentrando catálogos de tipos/áreas, sanitizers, carga/salvamento de tipos/áreas/tipos de classificação, filtros por perfil e campos de contexto por tipo de documento; `firestore-service.ts` permanece como fachada compatível para AdminPanel, Upload, NewDocument, geração e orquestrador V3. Validação local concluída: guardrail arquitetural (`325` arquivos fonte), typecheck, testes focados de Firestore/Admin Taxonomy/consumidores (**5 arquivos**, **62 testes**), suíte completa frontend (**71/71 arquivos**, **530/530 testes**) e build com `VITE_FIRESTORE_DATABASE_ID=lexio-prod` verdes. Restam exatamente 2 repositories planejados nesta fase: Dashboard Stats/Cost e Chat.
- Etapa 376: repository de Dashboard Stats/Cost extraído para `frontend/src/lib/modules/dashboard/repository.ts`, com builders puros em `frontend/src/lib/modules/dashboard/metrics.ts`, concentrando agregados de dashboard, séries diárias, estatísticas por tipo, documentos recentes, snapshot de dashboard e breakdown de custos; `firestore-service.ts` permanece como fachada compatível para Dashboard, AdminPanel e CostTokensPage. Validação local concluída: guardrail arquitetural (`327` arquivos fonte), typecheck, testes focados de Firestore/Dashboard/guardrails (**4 arquivos**, **62 testes**), suíte completa frontend (**71/71 arquivos**, **533/533 testes**) e build com `VITE_FIRESTORE_DATABASE_ID=lexio-prod` verdes. Resta exatamente 1 repository planejado nesta fase: Chat.
- Etapa 377: repository de Chat extraído para `frontend/src/lib/modules/chat/repository.ts`, concentrando conversas, turns, sidecar devices, workspace roots/bindings, sidecar commands, approvals e audit; `firestore-service.ts` permanece como fachada compatível para ConversationList, use-chat-controller e integrações do orquestrador. Validação local concluída: guardrail arquitetural (`329` arquivos fonte), typecheck, testes focados de Firestore/Chat/orquestrador (**10 arquivos**, **110 testes**), suíte completa frontend (**71/71 arquivos**, **537/537 testes**) e build com `VITE_FIRESTORE_DATABASE_ID=lexio-prod` verdes. Restam exatamente 0 repositories planejados nesta fase.
- Etapa 378: auditoria cautelosa de segurança/modularização pós-closeout reforçou guardrails e higiene de dados: o teste estrutural de auth/UID passou a incluir `frontend/src/lib/modules/chat/repository.ts`, a exclusão de conversa agora limpa subcoleções mutáveis de Chat sem violar audit append-only, o inventário de collection groups de migração passou a refletir todos os paths atuais de Chat e os documentos/env foram alinhados ao estado `lexio-prod` + PR #143 aberto para merge/deploy/smoke.
- Etapa 379: corte produtivo concluído em `main`: PR #143 mergeada (`15b32d1`), `Tests` (`25576968289`), Firebase production (`25576968354`) e release-web Firebase + GitHub Pages (`25581626099`) verdes; smoke público e autenticado real em `https://lexio.web.app` confirmou rotas críticas e 34 chamadas Firestore para `lexio-prod`, 0 para `(default)`, 0 erros de browser/Firestore; DataJud proxy validado com `tjrs` status 200 e logs de erro pós-release limpos.
