...
class Settings(BaseSettings):
    ...
    def require_non_empty(self, field_name: str, env_var_name: str | None = None) -> str:
        value = str(getattr(self, field_name, "") or "").strip()
        if value:
            return value

        env_name = env_var_name or field_name.upper()
        raise RuntimeError(f"{env_name} must be configured before using this backend capability.")
