...
- **Paralelização segura e adaptativa em pipelines críticos** — Etapas independentes agora rodam com concorrência controlada/adaptativa (base complementar do gerador documental, lotes do analista de acervo, batches de imagem e TTS no pipeline de vídeo), com caps por hardware, variáveis de tuning opcional e preservação de fallback/telemetria operacional
- **Painéis flutuantes endurecidos para mobile** — `DraggablePanel` recebeu geometria compacta automática em viewport estreita, clamp de posição/tamanho e guardrails de interação para estabilizar modais reutilizados em telas pequenas
