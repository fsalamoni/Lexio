...
- **Paralelização segura e adaptativa em pipelines críticos** — Etapas independentes agora rodam com concorrência controlada/adaptativa via política unificada (`runtime-concurrency`), com caps por hardware/memória/rede, diagnósticos por lote e telemetria persistida em `llm_executions` (`runtime_profile`, `runtime_hints`, `runtime_concurrency`, `runtime_cap`)
- **Painéis flutuantes endurecidos para mobile** — `DraggablePanel` recebeu geometria compacta automática em viewport estreita, clamp de posição/tamanho, guardrails de interação, leitura de `visualViewport` e suporte a safe-area (`env(safe-area-inset-*)`) para estabilizar modais com teclado virtual, notch e home indicator
